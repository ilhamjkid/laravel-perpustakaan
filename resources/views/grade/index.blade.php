...
@section('container')
    ...
    <div class="row">
        @auth
            <div class="col-auto mb-2">
                <a class="btn btn-primary" href="/grade/create">Tambah Kelas</a>
            </div>
        @endauth
                ...
                @if ($grades->count() > 0)
                    @foreach ($grades as $grade)
                        <tr class="align-middle">
                            ...
                            @auth
                                <td>
                                    <a class="badge bg-warning text-white" href="/grade/{{ $grade->slug }}/edit">
                                        <span data-feather="edit"></span>
                                    </a>
                                    <button type="button" id="{{ $grade->slug }}" data-grade="{{ $grade->name }}"
    ...
    <div class="modal fade" id="modalDelete" tabindex="-1" aria-labelledby="modalDeleteData" aria-hidden="true">
        <div class="modal-dialog">
            <form action="/grade" method="post" class="modal-content">
                @csrf
                @method('delete')
                <div class="modal-header">
                    <h5 class="modal-title" id="modalDeleteData">Hapus kelas</h5>
    ...
    <script>
        ...
        btnModal.forEach((button) => {
            button.addEventListener('click', function() {
                ...
                h5.innerHTML = 'Anda yakin ingin menghapus kelas ' +
                    this.dataset.grade;
                modal.action = '/grade/' + this.id;
            });
        });
