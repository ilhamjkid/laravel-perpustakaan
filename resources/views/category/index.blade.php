    ...
    <div class="row">
        @auth
            <div class="col-auto mb-2">
                <a class="btn btn-primary" href="/category/create">Tambah Kategori</a>
            </div>
        @endauth
                ...
                @if ($categories->count() > 0)
                    @foreach ($categories as $category)
                        <tr class="align-middle">
                            ...
                            @auth
                                <td>
                                    <a class="badge bg-warning text-white" href="/category/{{ $category->slug }}/edit">
                                        <span data-feather="edit"></span>
                                    </a>
                                    <button type="button" id="{{ $category->slug }}" data-category="{{ $category->name }}"
        ...
        <div class="modal-dialog">
            <form action="/category" method="post" class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalDeleteData">Hapus kategori</h5>
    ...
    <script>
        ...
        btnModal.forEach((button) => {
            button.addEventListener('click', function() {
                ...
                h5.innerHTML = 'Anda yakin ingin menghapus kategori ' +
                    this.dataset.category;
                modal.action = '/category/' + this.id;
            });
        });
